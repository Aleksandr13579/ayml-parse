...
class Compare {
    ...
    String whatHasBeenAdded( ) {
        ...
        allChanges.append("<br><font color=\"#4D6AFF\">Добавлены параметры:</font> ${newKey.length() != 0 ? "<font color=\"#FFB3B3\">${newKey}</font><br>" : "Различий нет<br>"} \n")
                .append("<font color=\"#4D6AFF\">Изменения в параметрах по значениям:</font> <br>")
        ...
        if (!differentValue.isEmpty()) {
            differentValue.each {key, value ->
                allChanges.append("В параметре " + "<font color=\"#FFB3B3\">${key}</font>" + " было "
                        + "<font color=\"#4D6AFF\">${this.dataFromFirstFile.get(key)}</font>" + " стало " + "<font color=\"#FFB3B3\">${value}</font>" + '<br>')
            }
        }
