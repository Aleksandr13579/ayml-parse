...
import kernel.geom.Subpath;
import kernel.pdf.canvas.PdfCanvasConstants;
 ...
 * For example:
 * <ul>
 *     <li>{@link PolyTree} to {@link kernel.geom.Path}
 *     <li>{@link kernel.geom.Point} to {@link Point.LongPoint}
 *     <li>{@link Point.LongPoint} to {@link kernel.geom.Point}
 * </ul>
 */
public final class ClipperBridge {
     ...
     * {@link kernel.geom.Path} object.
     *
     * @param result {@link PolyTree} object to convert
     * @return resultant {@link kernel.geom.Path} object
     */
    public static kernel.geom.Path convertToPath(PolyTree result) {
        kernel.geom.Path path = new kernel.geom.Path();
    ...
    /**
     * Adds iText {@link kernel.geom.Path} to the given {@link IClipper} object.
     * @param clipper The {@link IClipper} object.
     * @param path The {@link kernel.geom.Path} object to be added to the {@link IClipper}.
     * @param polyType See {@link IClipper.PolyType}.
     */
    public static void addPath(IClipper clipper, kernel.geom.Path path, IClipper.PolyType polyType) {
        for (Subpath subpath : path.getSubpaths()) {
            if (!subpath.isSinglePointClosed() && !subpath.isSinglePointOpen()) {
                List<kernel.geom.Point> linearApproxPoints = subpath.getPiecewiseLinearApproximation();
    ...
    /**
     * Adds all iText {@link Subpath}s of the iText {@link kernel.geom.Path} to the {@link ClipperOffset} object with one
     ...
     * @param path     {@link kernel.geom.Path} object, containing the required {@link Subpath}s
     ...
     */
    public static List<Subpath> addPath(ClipperOffset offset, kernel.geom.Path path, IClipper.JoinType joinType, IClipper.EndType endType) {
     ...
     * {@link kernel.geom.Point} objects.
     *
     * @param points the list of {@link Point.LongPoint} objects to convert
     * @return the resultant list of {@link kernel.geom.Point} objects.
     */
    public static List<kernel.geom.Point> convertToFloatPoints(List<Point.LongPoint> points) {
        List<kernel.geom.Point> convertedPoints = new ArrayList<>(points.size());

        for (Point.LongPoint point : points) {
            convertedPoints.add(new kernel.geom.Point(
    ...
    /**
     * Converts list of {@link kernel.geom.Point} objects into list of
     * {@link Point.LongPoint} objects.
     *
     * @param points the list of {@link kernel.geom.Point} objects to convert
     * @return the resultant list of {@link Point.LongPoint} objects.
     */
    public static List<Point.LongPoint> convertToLongPoints(List<kernel.geom.Point> points) {
        List<Point.LongPoint> convertedPoints = new ArrayList<>(points.size());

        for (kernel.geom.Point point : points) {
    ...
    /**
     * Adds polygon path based on array of {@link kernel.geom.Point} (internally converting
     ...
     * @param polyVertices an array of {@link kernel.geom.Point} which will be internally converted
     ...
     */
    public static boolean addPolygonToClipper(IClipper clipper, kernel.geom.Point[] polyVertices, IClipper.PolyType polyType) {
        return clipper.addPath(new Path(convertToLongPoints(new ArrayList<>(Arrays.asList(polyVertices)))), polyType, true);
    }

    /**
     * Adds polyline path based on array of {@link kernel.geom.Point} (internally converting
     * them by {@link #convertToLongPoints}) and adds this path to {@link IClipper} instance, treating the path as
     ...
     * @param lineVertices an array of {@link kernel.geom.Point} which will be internally converted
     *                     to clipper path and added to the clipper instance.
     * @return true if polyline path was successfully added, false otherwise.
     */
    public static boolean addPolylineSubjectToClipper(IClipper clipper, kernel.geom.Point[] lineVertices) {
    ...
    static void addContour(kernel.geom.Path path, List<Point.LongPoint> contour, boolean close) {
        List<kernel.geom.Point> floatContour = convertToFloatPoints(contour);
        kernel.geom.Point point = floatContour.get(0);
